...
def upload_test_photo_to_minio(filename: str) -> str:
    """Upload a test photo to MinIO certificates bucket and return the path."""
    import io
    ...
    try:
        ...
        file_id = f"{uuid_lib.uuid4()}_{filename}"

        # Determine content type
        ext = photo_path.suffix.lower()
        content_type = "image/png" if ext == ".png" else "image/jpeg"

        # Upload to MinIO certificates bucket
        ...
        # Return the path in format: certificates/file_id
        return f"certificates/{file_id}"

    except Exception as e:
        print(f"  ⚠️  Failed to upload {filename} to MinIO: {e}")
        return None
...
async def seed_certificates():
    ...
    for bucket in ["certificates", "cert-temp"]:
    ...
    # Upload test photos to MinIO
    print("📷 Uploading test photos to MinIO...")
    certificate_photo_url = upload_test_photo_to_minio("certificate-photo-example.png")
    front_logo_url = upload_test_photo_to_minio("front-logo-example.png")
    back_logo_url = upload_test_photo_to_minio("back-logo-example.png")

    if certificate_photo_url:
        print(f"  ✓ Certificate photo uploaded: {certificate_photo_url}")
    if front_logo_url:
        print(f"  ✓ Front logo uploaded: {front_logo_url}")
    if back_logo_url:
        print(f"  ✓ Back logo uploaded: {back_logo_url}")
    ...
    for cert_type in cert_types:
        ...
        for idx, client_name in enumerate(client_names, 1):
            ...
            cert_doc = {
                ...
                "photo_url": certificate_photo_url,  # MinIO path: certificates/file_id
                "brand_logo_url": front_logo_url,  # MinIO path: certificates/file_id
                "rear_brand_logo_url": back_logo_url,  # MinIO path: certificates/file_id
                "is_deleted": False,
                "created_at": datetime.utcnow(),
                "updated_at": datetime.utcnow(),
            }
