...
async def get_natural_diamond_defaults(
    ...
    if not doc:
        return {"hardness": "10", "sg": "3.52"}
    return {"hardness": doc.get("hardness", "10"), "sg": doc.get("sg", "3.52")}
...
async def update_natural_diamond_defaults(
    ...
    await db.management_settings.update_one(
        ...
        {"$set": {"key": NATURAL_DIAMOND_KEY, "hardness": hardness, "sg": sg, "updated_at": datetime.utcnow()}},
        upsert=True,
    )
    return {"hardness": hardness, "sg": sg}
