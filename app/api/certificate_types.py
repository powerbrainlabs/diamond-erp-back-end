...
from ..core.dependencies import require_super_admin, require_staff, get_org_scope, org_filter
...
async def list_certificate_types(
    ...
    db = await get_db()
    cursor = (
        db.certificate_types.find({"is_deleted": False, "is_active": True, **org_filter(scope)})
...
async def list_all_certificate_types(
    ...
    db = await get_db()
    cursor = (
        db.certificate_types.find({"is_deleted": False, **org_filter(scope)})
        .sort([("display_order", 1)])
    )
    return {"data": [serialize_type(doc) async for doc in cursor]}
...
async def create_certificate_type(
    ...
    existing = await db.certificate_types.find_one({
        "slug": payload.slug, "is_deleted": False, **org_filter(scope),
    })
    if existing:
        raise HTTPException(status_code=409, detail="A type with this slug already exists")

    # Determine next display_order (within the organization)
    last = await db.certificate_types.find_one(
        {"is_deleted": False, **org_filter(scope)}, sort=[("display_order", -1)]
    )
...
async def update_certificate_type(
    ...
    db = await get_db()
    doc = await db.certificate_types.find_one({"uuid": type_uuid, "is_deleted": False, **org_filter(scope)})
    if not doc:
...
async def reorder_certificate_types(
    ...
    for idx, type_uuid in enumerate(payload.type_order):
        await db.certificate_types.update_one(
            {"uuid": type_uuid, "is_deleted": False, **org_filter(scope)},
